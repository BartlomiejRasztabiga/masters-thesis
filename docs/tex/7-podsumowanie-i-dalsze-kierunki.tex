...
Wybór modelu powinien być podyktowany profilem ryzyka wdrożenia: \texttt{gpt-5-mini} dla środowisk zaufanych wymagających najwyższej skuteczności, \texttt{gemini-2.5-flash} dla środowisk, gdzie odporność na manipulację jest priorytetem. \texttt{DeepSeek V3.2} nie jest zalecany w środowiskach produkcyjnych ze względu na pełną podatność w testach manipulacji kontekstem.

\subsection{Ograniczenia pracy}

Wyniki dotyczą konkretnej implementacji systemu agentowego, określonego zestawu modeli komercyjnych i zbioru prostych aplikacji webowych. Nie można ich bezpośrednio uogólniać na inne technologie, modele open-source ani środowiska produkcyjne różniące się od lokalnego klastra testowego.

Kluczowe ograniczenia generalizowalności wyników obejmują: zależność od konkretnego promptu i zestawu narzędzi agenta (inne implementacje mogą dać istotnie różne rezultaty), brak walidacji runtime w eksperymentach RQ3 i RQ5 (ocena jakości i podatności opiera się wyłącznie na analizie statycznej artefaktów) oraz brak pętli sprzężenia zwrotnego z rzeczywistym środowiskiem wykonawczym — architektura agenta generującego konfiguracje statycznie może zaniżać wyniki względem systemów z iteracyjną korektą. Wreszcie, eksperymenty przeprowadzono na aplikacjach webowych w językach Python i Node.js; wyniki nie charakteryzują innych stosów technologicznych ani typów systemów (np.\ aplikacje bazodanowe, systemy czasu rzeczywistego). Szczegółowa retrospektywna ocena zagrożeń trafności znajduje się w rozdziale~6.
