...
\item sformułowanie pytań badawczych dotyczących możliwości, ograniczeń oraz zagrożeń związanych z autonomicznym generowaniem konfiguracji IaC przez agenty LLM,
...
\item automatyczną ocenę jakości generowanych konfiguracji z użyciem narzędzi statycznej analizy,
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Rozdział 2 – Przegląd literatury}: analiza aktualnych badań nad wykorzystaniem LLM w generowaniu konfiguracji IaC (Docker, Kubernetes), naprawa błędów konfiguracyjnych oraz identyfikacja luk w istniejącej wiedzy,
    \item \textbf{Rozdział 3 – Przegląd technologii i narzędzi}: opis wybranych modeli językowych (Gemini 2.5 Flash, GPT-5 Mini, DeepSeek V3.2), narzędzi walidacji statycznej (Hadolint, Kube-linter) oraz architektury zbudowanego systemu agentowego,
    \item \textbf{Rozdział 4 – Projekt eksperymentów}: sformułowanie pięciu pytań badawczych (RQ1--RQ5), opis metodyki, zbiorów danych testowych (25 repozytoriów oraz repozytoria kontrolowane POC1--POC5), metryk oceny oraz zagrożeń trafności,
    \item \textbf{Rozdział 5 – Wyniki eksperymentów}: szczegółowa prezentacja wyników dla każdego z pięciu pytań badawczych wraz z analizą ilościową, tabelami i typowymi przyczynami awarii,
    \item \textbf{Rozdział 6 – Dyskusja wyników}: przekrojowa interpretacja rezultatów obejmująca syntezę RQ1--RQ5, porównanie modeli, rolę prompt engineeringu, niezawodność procesu agentowego, aspekty bezpieczeństwa, odniesienie do literatury oraz implikacje praktyczne,
    \item \textbf{Rozdział 7 – Podsumowanie i dalsze kierunki badań}: zwięzłe odpowiedzi na pytania badawcze, kluczowe wnioski, ocena przydatności agentów LLM w DevOps oraz kierunki dalszych badań.
\end{itemize}
