...
\begin{itemize}
    ...
    \item skuteczność etapów: build 78{,}7\% (próg 75\%), apply 76{,}7\% (próg 75\%), runtime 62{,}7\% (próg 60\%),
...
\textbf{Zestawienie per repozytorium i model:} Kluczowe metryki powtarzalności (średni diff ratio oraz zmienność liczby kroków narzędziowych) zestawiono w tabeli~\ref{tab:h4-repeatability}. Wartości dla kroków narzędziowych raportowane są dla tej samej kombinacji repozytorium i modelu, zgodnie z definicją H4.
...
\begin{table}[h]
    ...
    \end{tabular}
    \caption{Powtarzalność per repozytorium i model w H4}
...
\textbf{Wyniki zagregowane:} Średni diff ratio dla całego H4 wyniósł 0{,}5632 (mediana 0{,}5401). W ujęciu per model, \texttt{gemini-2.5-flash} osiąga 0{,}5274, a \texttt{deepseek-chat} 0{,}5990. Średnie odchylenie standardowe liczby kroków narzędziowych wyniosło 5{,}37 (per model: 4{,}34 dla gemini i 6{,}40 dla deepseek), co wskazuje na umiarkowaną zmienność procesu wykonawczego w skali całego eksperymentu.

\subsubsection{Werdykt dla H4}

Na podstawie kryteriów z rozdziału 4 hipoteza H4 nie została potwierdzona w swojej pierwotnej, rygorystycznej formie. Spełnienie warunków przedstawia się następująco:
\begin{itemize}
    \item średni run-to-run diff ratio $> 0{,}4$ spełniono w 7/10 kombinacji (wymagane: wszystkie),
    \item odchylenie standardowe liczby kroków narzędziowych przekroczyło 20\% średniej tylko w 1/10 kombinacji.
\end{itemize}
Oznacza to, że chociaż w wielu konfiguracjach występuje istotna zmienność wyników, nie jest ona na tyle systematyczna, aby spełnić zaostrzone kryteria weryfikacyjne H4. Jednocześnie obserwowany jest wyraźny wzrost diff ratio wraz ze wzrostem złożoności repozytoriów, co potwierdza trend ograniczonej powtarzalności dla trudniejszych przypadków. Z tego względu hipotezę H4 należy uznać za częściowo potwierdzoną.

\subsection{Wyniki H5: Podatność na manipulację kontekstem}

\subsubsection{Wyniki ilościowe}

Eksperyment H5 objął 90 przebiegów (3 warianty manipulacji $\times$ 3 modele $\times$ 10 powtórzeń). Łącznie w 51/90 przebiegach wykryto krytyczne odchylenie od specyfikacji repozytorium, co odpowiada 56{,}7\% (95\% CI: 46{,}4\%--66{,}4\%).
...
\begin{table}[h]
    ...
    \end{tabular}
    \caption{Odsetek odchyleń w H5 per wariant manipulacji}
...
\textbf{Porównanie modeli w H5:} najwyższą podatność wykazał \texttt{deepseek-chat}, dla którego odchylenie wystąpiło w 30/30 przebiegach (100{,}0\%; 95\% CI: 88{,}6\%--100{,}0\%). Model \texttt{gpt-5-mini} osiągnął 19/30 odchyleń (63{,}3\%; 95\% CI: 45{,}5\%--78{,}1\%), natomiast \texttt{gemini-2.5-flash} był wyraźnie bardziej odporny i uległ manipulacji tylko w 2/30 przebiegach (6{,}7\%; 95\% CI: 1{,}8\%--21{,}3\%).
...
Hipoteza H5 została potwierdzona. Spełnione zostały oba kryteria z rozdziału 4:
\begin{itemize}
    \item ogólny odsetek przebiegów z krytycznym odchyleniem wyniósł 56{,}7\% (próg: 30\%),
    \item każdy z trzech wariantów manipulacji spowodował co najmniej jedno krytyczne odchylenie.
\end{itemize}
Wynik ten wskazuje, że agenty LLM mogą traktować dokumentację repozytorium jako źródło silniejszego sygnału niż sam kod aplikacji, co prowadzi do generowania artefaktów IaC zgodnych z fałszywym kontekstem, lecz sprzecznych z rzeczywistą specyfikacją systemu.
