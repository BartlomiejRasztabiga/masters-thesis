...
Wyniki dla repozytoriów POC1--POC4 zestawiono w tabeli~\ref{tab:h2-repos}.
...
Wraz ze wzrostem złożoności widać istotny spadek skuteczności: od 100{,}0\% dla POC1 do 33{,}3\% dla POC4.

% TODO: Wykres success rate vs. poziom złożoności (repozytoria kontrolowane).

\subsubsection{Trend złożoności}

Dane wskazują na silny trend spadkowy skuteczności wraz ze wzrostem złożoności (korelacja Spearmana $\rho \approx -0{,}95$ dla POC1--POC4). Największy spadek widoczny jest między POC1 i POC2, następnie obserwowany jest plateau na poziomie POC2--POC3, po czym skuteczność spada dalej dla POC4. Wskazuje to na punkt załamania po przekroczeniu prostej architektury wielowarstwowej, gdy dochodzą dodatkowe zależności infrastrukturalne i wzajemne powiązania usług.

\subsubsection{Werdykt dla H2}

Na podstawie wyników dla POC1--POC4 hipoteza H2 jest potwierdzona w zakresie dostępnych danych: spełniony jest warunek trendu spadkowego ($\rho \leq -0{,}6$), a różnica skuteczności POC1--POC4 wynosi 66{,}7 p.p. Pełna weryfikacja zgodna z kryteriami z rozdziału 4 wymaga uzupełnienia wyników dla POC5.
