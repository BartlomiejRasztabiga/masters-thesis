...
\begin{itemize}
    \item \textbf{DeepSeek-Chat (V3.2)} \cite{deepseek_v32}
\end{itemize}

Wszystkie wymienione modele reprezentują czołówkę współczesnych dużych modeli językowych. Wybrane zostały przede wszystkim ze względu na ich potwierdzone zdolności do generowania wysokiej jakości kodu oraz wsparcie dla funkcji \textit{tool calling}. Funkcje te umożliwiają bezpośrednią integrację modeli z zewnętrznymi narzędziami lub usługami poprzez dedykowane mechanizmy API bądź platformy agentowe, takie jak LangChain \cite{langchain} czy LangGraph \cite{langgraph}. W proponowanym w tej pracy podejściu funkcje \textit{tool calling} odgrywają znaczącą rolę w zarządzaniu kontekstem i procesie generowania konfiguracji.
...
\begin{itemize}
	...
	\item \textbf{Chain-of-Thought (CoT) prompting:} Jest to technika, w której prompt instruuje model, aby przed podaniem ostatecznej odpowiedzi, wygenerował sekwencję pośrednich kroków rozumowania. Model "myśli na głos", co często prowadzi do bardziej precyzyjnych i trafnych wyników, szczególnie w zadaniach wymagających złożonych rozumowań lub wieloetapowych rozwiązań. W początkowej fazie rozwoju LLM technika CoT często wymagała dedykowanych instrukcji w prompcie, aby model wygenerował te kroki. Warto jednak zaznaczyć, że opisana zdolność do jawnego generowania kroków pośrednich dotyczy przede wszystkim wyspecjalizowanych modeli klasy \textit{reasoning} (takich jak OpenAI o1/o3 czy DeepSeek-R1), które posiadają dedykowane mechanizmy ukrytego wnioskowania trenowane osobno. Standardowe modele konwersacyjne — w tym modele wykorzystywane w niniejszej pracy (\texttt{GPT-5 Mini}, \texttt{Gemini 2.5 Flash}, \texttt{DeepSeek V3.2}) — generują odpowiedź token po tokenie bez gwarantowanego, automatycznego kroku rozumowania pośredniego; CoT pełni w ich przypadku rolę techniki promptowania, a nie wbudowanego mechanizmu modelu. W kontekście IaC, model mógłby najpierw "zastanowić się" nad zależnościami aplikacji, potem nad wymaganiami środowiskowymi i optymalizacjami, a dopiero potem wygenerować kod konfiguracji. CoT poprawia transparentność i debugowalność procesu generowania, a także może zwiększyć poprawność odpowiedzi.
	\item \textbf{Agent-based prompting:} Ten tryb wykracza poza jednorazowe generowanie odpowiedzi. Model, działając jako "agent", otrzymuje zadanie i może iteracyjnie wykonywać szereg akcji: planować, używać zewnętrznych narzędzi (tzw. "tool calling"), analizować ich wyniki i modyfikować swój plan działania w celu osiągnięcia celu. Podejście agentowe można postrzegać jako ewolucję i rozszerzenie idei Chain-of-Thought, gdzie "myślenie" modelu jest przekształcane w sekwencję interakcji ze światem zewnętrznym i refleksji. Agent nie tylko generuje wewnętrzne kroki rozumowania, ale także aktywnie wykorzystuje narzędzia do zbierania danych, sprawdzania założeń i korygowania swojego działania na podstawie rzeczywistych wyników. Jest to podejście szczególnie efektywne w przypadku złożonych problemów wymagających interakcji ze środowiskiem zewnętrznym oraz adaptacji.
\end{itemize}
...
W niniejszej pracy magisterskiej zastosowano właśnie metodę agentową. Podejście agentowe może być realizowane na różne sposoby, zarówno poprzez bezpośrednie wykorzystanie dedykowanych interfejsów API udostępnianych przez niektóre modele (np. poprzez \textit{tool calling} czy \textit{function calling}), jak i poprzez zastosowanie specjalizowanych frameworków orkiestracyjnych, takich jak \textbf{AutoGen} \cite{autogen} czy \textbf{LangChain/LangGraph}. W tym badaniu do implementacji agentów i zarządzania złożonymi przepływami pracy z wykorzystaniem funkcji \textit{tool calling} wybrana została biblioteka LangGraph w języku Python, uzupełniona o funkcjonalności LangChain. Rola agenta koncentruje się na efektywnym dostarczaniu kontekstu i zarządzaniu procesem generacji konfiguracji IaC.
...
W kontekście automatycznego generowania konfiguracji Infrastructure as Code (IaC) przez modele językowe ważne jest zapewnienie, że wygenerowane konfiguracje są nie tylko poprawne składniowo, ale również bezpieczne, zgodne z najlepszymi praktykami i efektywne. Do tego celu służą wyspecjalizowane narzędzia do analizy i walidacji kodu IaC. Wyniki uzyskane z tych narzędzi stanowią podstawę do obiektywnej oceny jakości konfiguracji generowanych przez LLM w dalszej części niniejszej pracy. W niniejszym podrozdziale opisano dwa wybrane narzędzia użyte w badaniu: Hadolint \cite{hadolint} (dla Dockerfile) i Kube-linter \cite{kubelinter} (dla manifestów Kubernetes).
...
Interpretacja wyników z obu narzędzi wymaga zrozumienia specyfiki Dockerfile i manifestów Kubernetes. Ważne jest, aby skupić się na błędach o wysokim poziomie ważności (np. problemy z bezpieczeństwem, brak sond) oraz na ostrzeżeniach, które mogą prowadzić do problemów z wydajnością lub stabilnością aplikacji. Wyjścia z tych narzędzi zostały wykorzystane w dalszej części pracy do obiektywnej oceny jakości generowanych konfiguracji IaC.

\subsection{Środowisko eksperymentalne i technologie wspierające}

Eksperymenty zostały przeprowadzone na lokalnej stacji roboczej z systemem operacyjnym Debian GNU/Linux, wyposażonej w 16 GB pamięci RAM. W środowisku tym nie wykorzystywano lokalnie żadnych kart graficznych (GPU) do inferencji modeli językowych. Wszystkie interakcje z modelami LLM odbywały się wyłącznie za pośrednictwem zewnętrznych interfejsów API.

Do zarządzania kontenerami i środowiskami uruchomieniowymi wykorzystano następujące narzędzia:
\begin{itemize}
	\item \textbf{Docker:} \cite{merkel_docker_nodate} Używany do uruchamiania kontenerów oraz do budowania obrazów Docker na podstawie generowanych plików Dockerfile. Docker służył jako podstawowe środowisko konteneryzacji.
	\item \textbf{MicroK8s:} \cite{microk8s} Lekka dystrybucja Kubernetes przeznaczona do uruchamiania w środowiskach produkcyjnych oraz testowych. MicroK8s został zainstalowany bezpośrednio na hoście z systemem Debian i wykorzystany jako klaster Kubernetes umożliwiający walidację wygenerowanych konfiguracji w warunkach zbliżonych do rzeczywistych wdrożeń.
\end{itemize}

Cała logika sterowania eksperymentami, w tym implementacja agentów, wywoływanie narzędzi, walidacja i zbieranie wyników, została zrealizowana przy użyciu języka \textbf{Python (3.11+)}. Wykorzystane komponenty obejmowały:
\begin{itemize}
    ...
    \item \textbf{Narzędzia CLI Docker i Kubernetes:} Do walidacji i uruchamiania konfiguracji wykorzystywano polecenia \texttt{docker} (m.in. \texttt{buildx} oraz \texttt{push}) oraz \texttt{kubectl} (walidacja \texttt{--dry-run}, wdrażanie manifestów i sprawdzanie stanu zasobów w klastrze).
    \item \textbf{PyYAML:} Wykorzystywany do parsowania i modyfikacji manifestów Kubernetes w celu ich dostosowania do kontekstu uruchomienia (np. tagów obrazów i przestrzeni nazw).
...
\begin{itemize}
    \item \textbf{Bezpośrednie API:} W przypadku modeli komercyjnych, które oferują własne dedykowane API (np. OpenAI GPT-5 Mini, Google Gemini 2.5 Flash), korzystano z ich natywnych interfejsów programistycznych.
    \item \textbf{OpenRouter:} Dla modeli open-source (np. Mistral, Meta Llama), które często nie posiadają własnych komercyjnych API lub ich infrastruktura do hostingu jest zbyt kosztowna dla pojedynczego badacza, wykorzystano platformę \textbf{OpenRouter}. OpenRouter działa jako brama API (API Gateway), agregująca dostęp do wielu modeli od różnych dostawców za pośrednictwem ujednoliconego interfejsu API, co znacząco ułatwiało testowanie różnorodnych modeli bez konieczności integracji z wieloma odrębnymi API.
\end{itemize}
