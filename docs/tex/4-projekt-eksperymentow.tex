...
Agent został zaimplementowany przy użyciu biblioteki LangGraph i pełni rolę autonomicznego generatora konfiguracji. Otrzymuje on link do repozytorium Git oraz systemowy prompt z instrukcjami i ma za zadanie wygenerować kompletne pliki \texttt{Dockerfile} oraz manifesty Kubernetes.
...
\begin{itemize}
    \item \texttt{clone\_repo(repo\_url)} – klonuje repozytorium Git i automatycznie usuwa pliki mogące sugerować gotowe rozwiązanie: metadane VCS (\texttt{.git}, \texttt{.github}, \texttt{.gitignore}), dokumentację (\texttt{README.md}, \texttt{LICENSE}), istniejące konfiguracje (\texttt{Dockerfile}, \texttt{docker-compose.yml}), pliki CI/CD (\texttt{.gitlab-ci.yml}, \texttt{.travis.yml}) oraz artefakty deweloperskie (\texttt{\_\_pycache\_\_}, \texttt{.pytest\_cache}, \texttt{.idea}, \texttt{.vscode}). Usunięcie istniejących plików \texttt{Dockerfile} i \texttt{docker-compose.yml} jest celową decyzją — zapewnia, że agent generuje konfiguracje wyłącznie na podstawie analizy kodu źródłowego,
    ...
    \item \texttt{base64\_encode(content)} – koduje tekst do base64 (dla Kubernetes Secrets),
...
Wszystkie testowane modele otrzymują identyczny prompt oraz to samo repozytorium testowe, co zapewnia warunki umożliwiające rzetelne porównanie. Parametry wywołania:
...
Ze względu na koszt wywołań API oraz objętość danych, dla H1 przeprowadzane są 1–2 powtórzenia dla każdego repozytorium z 2–3 najlepszymi modelami komercyjnymi (np. GPT-5, Claude Sonnet 4.5).
...
\begin{itemize}
    \item \textbf{Pozytywna weryfikacja hipotezy}: Success rate > 60–70\% na różnorodnym zbiorze projektów — wskazuje, że agenty LLM potrafią autonomicznie generować funkcjonalne konfiguracje w większości przypadków.
    \item \textbf{Negatywna weryfikacja hipotezy}: Success rate < 50\% — wskazuje na fundamentalne ograniczenia podejścia agentowego w praktycznym zastosowaniu.
\end{itemize}
...
\bigskip
\noindent
Kontrolowane środowisko pozwala na precyzyjne porównanie wyników między różnymi poziomami złożoności. Testy obejmują 3–5 powtórzeń dla każdej kombinacji (model, repozytorium) z 4–6 modelami (commercial + open-source).
...
Model przypisuje różne wagi ważności poszczególnym fazom walidacji — fazy krytyczne dla funkcjonalności (składnia, budowa obrazu, aplikowanie do klastra) mają wyższe wagi (35–40\%) niż weryfikacja dobrych praktyk przez lintery (20–25\%).
...
\begin{itemize}
    ...
    \item \textbf{Scoring metrics}: overall\_score, dockerfile\_score, k8s\_score,
