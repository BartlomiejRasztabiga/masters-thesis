...
\begin{enumerate}
    \item Weryfikacja syntaktyczna \texttt{Dockerfile} (np. \texttt{docker build --check}),
    \item Statyczna analiza \texttt{Dockerfile} (Hadolint),
    \item Budowa obrazów Docker,
    \item Weryfikacja syntaktyczna manifestów Kubernetes (np. \texttt{kubectl apply --dry-run=server}),
...
\begin{itemize}
    ...
    \item \textbf{Rejestr Docker}: lokalny rejestr dla przechowywania zbudowanych obrazów (adres konfigurowany przez \texttt{REGISTRY\_URL}; domyślnie \texttt{192.168.0.124:32000}),
...
Wszystkie testowane modele otrzymują identyczny prompt oraz to samo repozytorium testowe, co zapewnia spójne warunki oceny wpływu modelu na jakość generacji IaC. Parametry wywołania (H1):

\begin{itemize}
    \item \texttt{temperature = 1.0} (zgodnie z konfiguracją eksperymentu),
    \item \texttt{recursion\_limit = 200} — limit iteracji w pętli agentowej,
    \item \textbf{powtórzenia}: 2 przebiegi dla każdej kombinacji (repozytorium, model).
\end{itemize}
...
W celu oceny ogólnej zdolności agentów LLM do generowania działających konfiguracji, testy H1 przeprowadzane są na \textbf{szerokim zestawie rzeczywistych, publicznie dostępnych projektów z GitHub}. Wykorzystanie niekontrolowanych repozytoriów o zróżnicowanej strukturze, niekompletnej dokumentacji i niestandardowych zależnościach pozwala ocenić skuteczność agentów w warunkach zbliżonych do praktycznych.

Dataset w ramach H1 obejmuje 25 aplikacji webowych (zgodnie z plikiem \texttt{experiments/h1.yaml}) zróżnicowanych pod względem stacku technologicznego oraz sposobu uruchamiania.

\textbf{Ograniczenia praktyczne:} w badaniu H1 celowo wykorzystano zestaw relatywnie prostych aplikacji (mało komponentów, brak skomplikowanej orkiestracji), aby umożliwić tanią i szybką ewaluację. Bardziej złożone przypadki są analizowane w H2. Liczba powtórzeń na kombinację (model, repozytorium) została ograniczona do 2 ze względu na budżet obliczeniowy — należy to uwzględnić przy interpretacji niepewności oszacowań oraz różnic między modelami.

\bigskip
\noindent
\textbf{Konfiguracja eksperymentu H1:} testy uruchamiane są z domyślnym promptem systemowym (\texttt{prompts/default.prompt}) oraz trzema modelami faktycznie użytymi w eksperymencie: \texttt{gemini-2.5-flash}, \texttt{gpt-5-mini} i \texttt{deepseek-chat}. Każda kombinacja (repozytorium, model) jest uruchamiana dwukrotnie.
...
\[
\text{Success rate} = \frac{\text{build\_success} \land \text{runtime\_success}}{\text{total\_runs}} \times 100\%
\]

W analizie dodatkowo raportowany jest spadek skuteczności na kolejnych etapach potoku walidacyjnego (składnia → build → deploy → runtime), aby wskazać typowe przyczyny niepowodzeń.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item większość porażek wyjaśnialna przez powtarzające się kategorie błędów (np. błędy budowy obrazu lub niekompletne manifesty), co pozwala na dalsze usprawnienie logiki agenta i procedur walidacji.
    \end{itemize}
...
\begin{itemize}
    ...
    \item dominacja awarii na etapach bazowych (składnia, build) — sugeruje, że agent nie radzi sobie z najprostszymi wymaganiami wdrożeniowymi w badanej klasie projektów.
    \end{itemize}
\end{itemize}

\textbf{Raportowanie w przypadku niskiego sukcesu E2E:} jeżeli współczynnik sukcesu end-to-end jest niski (np. kilkadziesiąt procent), w analizie prezentowane są również wskaźniki per etap (build/apply/runtime) oraz drop-off między etapami, aby wskazać, gdzie najczęściej dochodzi do awarii.
...
Aby ograniczyć wpływ losowych fluktuacji, proces generacji i ewaluacji może być powtarzany wielokrotnie dla każdej kombinacji (model, repozytorium, prompt). W H1 zastosowano 2 powtórzenia, co pozwala oszacować stabilność wyników przy rozsądnym koszcie eksperymentu.
...
\begin{itemize}
    \item \textbf{Metadata}: experiment, timestamp, repo\_url, repo\_name, model\_provider, model\_name, model\_label, temperature, seed, prompt\_id, repetition, status,
    \item \textbf{Generation metrics}: generation\_success, generation\_time, tool\_calls, tokens\_used,
...
Wszystkie interakcje agenta z modelem językowym są automatycznie śledzone przez platformę LangSmith, która umożliwia inspekcję pełnych ścieżek wywołań narzędzi, przesłanych promptów oraz odpowiedzi modelu.
