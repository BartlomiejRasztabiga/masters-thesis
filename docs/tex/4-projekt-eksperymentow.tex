...
\textbf{Przedziały ufności:} dla wskaźników proporcji (success rate, metryki per etap i warunkowe) stosowane są 95\% przedziały ufności wyznaczane metodą Clopper–Pearsona (implementacja w skrypcie \texttt{scripts/summarize\_experiment.py}), co jest konserwatywnym podejściem dla niewielkich prób.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item conditional apply $|$ build $\geq$ 95\% oraz conditional runtime $|$ apply $\geq$ 80\% — wskazuje, że główne awarie nie wynikają z błędów deploy/runtime przy poprawnie zbudowanych obrazach i zaaplikowanych manifestach,
        ...
        \item brak dominującej pojedynczej klasy błędów w fazie build/apply (pojedyncze repozytoria nie blokują wyniku globalnego).
    \end{itemize}
\end{itemize}
...
\subsubsection{Zagrożenia ważności eksperymentów}
\begin{itemize}
    ...
    \item \textbf{Ograniczona liczba powtórzeń}: 2 przebiegi na kombinację (repozytorium, model) dają szerokie przedziały ufności i niską moc testów przy porównaniach modeli/promptów.
    ...
    \item \textbf{Opcjonalny tracing}: brak włączonego LangSmith w części przebiegów ogranicza możliwość analizy zachowania agenta i może utrudniać wyjaśnienie niepowodzeń.
\end{itemize}
