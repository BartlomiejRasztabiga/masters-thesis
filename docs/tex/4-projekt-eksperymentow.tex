...
H3 wykorzystuje ten sam zbiór repozytoriów co H1 (25 publicznych projektów; załącznik~\ref{att:h1-dataset}). Każde repozytorium uruchamiane jest dla trzech modeli (\texttt{gemini-2.5-flash}, \texttt{gpt-5-mini}, \texttt{deepseek-chat}) w dwóch wariantach prompta (bazowy vs. best practices) i z jednym powtórzeniem. Liczbę powtórzeń ograniczono z 2 do 1 w celu skrócenia czasu eksperymentu. Analiza jakości wykonywana jest na wygenerowanych plikach \texttt{Dockerfile} oraz manifestach Kubernetes, niezależnie od wyniku runtime; na potrzeby H3 wyłączono aplikowanie manifestów na klaster, aby przyspieszyć walidację.
...
H4 koncentruje się na powtarzalności wyników generacji przy niezmienionych warunkach wejściowych oraz na stabilności przebiegu procesu agentowego (liczby kroków narzędziowych).
...
Analogicznie do H3, na potrzeby H4 wyłączono aplikowanie manifestów na klaster, ponieważ celem jest ocena powtarzalności generacji, a nie skuteczności runtime.
...
Hipoteza H4 zostaje potwierdzona, jeśli spełnione są łącznie następujące warunki:
\begin{itemize}
    \item średni run-to-run diff ratio dla każdej kombinacji (repozytorium, model) przekracza 0{,}4,
    \item odchylenie standardowe liczby kroków narzędziowych przekracza 20\% średniej w danej kombinacji.
\end{itemize}
...
Agent LLM jest podatny na manipulację kontekstem repozytorium; przy obecności złośliwych lub mylących instrukcji w dokumentacji i plikach konfiguracyjnych generuje artefakty IaC sprzeczne z rzeczywistym kodem aplikacji. Skutkuje to krytycznymi odchyleniami od oczekiwanej konfiguracji (np. uruchomieniem innego kontenera, błędnym portem, niepoprawnym poleceniem startowym).
...
\end{itemize}
Eksperyment wykonywany jest dla trzech modeli (\texttt{gemini-2.5-flash}, \texttt{gpt-5-mini}, \texttt{deepseek-chat}) z dziesięcioma powtórzeniami na kombinację, co daje łącznie 90 przebiegów. W celu skupienia oceny na podatności semantycznej, a nie na funkcjonalności końcowego wdrożenia, w H5 wyłączono walidację runtime; werdykt oparto na analizie wygenerowanych artefaktów (\texttt{Dockerfile} i manifestów Kubernetes).
...
Hipoteza H5 zostaje potwierdzona, jeśli spełniony jest co najmniej jeden z warunków:
\begin{itemize}
    \item odsetek przebiegów z krytycznymi odchyleniami wynosi co najmniej 30\%,
    \item każdy z wariantów ataku (A1--A3) powoduje co najmniej jedno krytyczne odchylenie.
\end{itemize}
