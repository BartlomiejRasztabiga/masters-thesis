...
\begin{itemize}
    ...
    \item \textbf{POC4:} prosty układ mikroserwisowy (3 usługi FastAPI), baza danych per serwis, broker komunikatów RabbitMQ, współdzielone biblioteki i zwiększona liczba zależności między usługami.
    \item \textbf{POC5:} rozszerzony układ mikroserwisowy (3 usługi FastAPI + 1 usługa Node.js), baza danych per serwis, RabbitMQ oraz Redis, dodatkowe mechanizmy uruchomieniowe (frontend czekający na zdrowie usług przez \texttt{/health}).
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{Poziom złożoności} (1–5) — przypisany zgodnie z POC1–POC5,
        \item \textbf{Liczba usług/kontenerów} — liczba uruchamianych komponentów aplikacyjnych,
        \item \textbf{Liczba baz danych} — liczba niezależnych magazynów danych,
        \item \textbf{Liczba zależności infrastrukturalnych} — komponenty inne niż bazy (np. broker).
    \end{itemize}
    \item \textbf{Metryki skuteczności}:
    \begin{itemize}
        \item end-to-end success rate (build \& apply \& runtime) per repozytorium i per poziom złożoności,
