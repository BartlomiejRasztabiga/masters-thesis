...
\textbf{Ograniczenia praktyczne:} w badaniu RQ1 znalazł się zestaw relatywnie prostych aplikacji (mało komponentów, brak skomplikowanej orkiestracji), co obniża koszt eksperymentu, ale ogranicza uogólnienie wyników na złożone systemy — jest to jedno z zagrożeń ważności opisanych w sekcji o threats to validity.
...
Każde repozytorium jest uruchamiane dla trzech modeli (\texttt{gemini-2.5-flash}, \texttt{gpt-5-mini}, \texttt{deepseek-chat}) z dwoma powtórzeniami, analogicznie do RQ1, aby umożliwić porównywalność wyników.
...
RQ3 wykorzystuje ten sam zbiór repozytoriów co RQ1 (25 publicznych projektów; załącznik~\ref{att:h1-dataset}). Każde repozytorium uruchamiane jest dla trzech modeli (\texttt{gemini-2.5-flash}, \texttt{gpt-5-mini}, \texttt{deepseek-chat}) w dwóch wariantach prompta (bazowy vs. best practices) i z jednym powtórzeniem. Liczbę powtórzeń ograniczono z 2 do 1 w celu skrócenia czasu eksperymentu. Analiza jakości wykonywana jest na wygenerowanych plikach \texttt{Dockerfile} oraz manifestach Kubernetes, niezależnie od wyniku runtime; na potrzeby RQ3 wyłączono aplikowanie manifestów na klaster, aby przyspieszyć walidację.
...
Wykorzystano pięć kontrolowanych repozytoriów POC (POC1--POC5), obejmujących rosnącą złożoność architektury. Eksperyment realizowany jest dla dwóch modeli (\texttt{gemini-2.5-flash}, \texttt{deepseek-chat}) i pojedynczego wariantu prompta (bazowego). Każda kombinacja (repozytorium, model) uruchamiana jest pięciokrotnie, co daje łącznie 50 przebiegów. We wszystkich powtórzeniach stosuje się identyczne parametry wejściowe (temperature=0, stały seed), aby mierzyć wahania wynikające z niedeterministycznych elementów procesu.

Wybór modeli podyktowany był koniecznością wsparcia parametru \texttt{temperature=0}. Modele z serii GPT-5 (w~tym \texttt{gpt-5-mini}) nie pozwalają na ustawienie temperatury innej niż domyślna wartość 1{,}0, ponieważ ich wewnętrzny proces generacji opiera się na wieloetapowym rozumowaniu (reasoning), które wymagałoby zredukowania wszystkich ścieżek rozumowania do jednej ścieżki zachłannej przy temperature=0, co uniemożliwiłoby wielokrotne próby rozwiązania problemu. Z~tego względu zastosowano modele \texttt{gemini-2.5-flash} (Google) oraz \texttt{deepseek-chat} (DeepSeek), które wspierają pełen zakres wartości temperatury (0--2) i~parametr \texttt{seed} dla deterministycznej generacji.
...
\end{itemize}
Eksperyment wykonywany jest dla trzech modeli (\texttt{gemini-2.5-flash}, \texttt{gpt-5-mini}, \texttt{deepseek-chat}) z dziesięcioma powtórzeniami na kombinację, co daje łącznie 90 przebiegów. W celu skupienia oceny na podatności semantycznej, a nie na funkcjonalności końcowego wdrożenia, w RQ5 wyłączono walidację runtime; analiza opiera się na wygenerowanych artefaktach (\texttt{Dockerfile} i manifestach Kubernetes).
