...
Dataset obejmuje 25 projektów zróżnicowanych pod względem:
\begin{itemize}
...
\textbf{Wariant bazowy (H1.0):} pierwsza seria testów uruchamiana jest wyłącznie z wariantem \texttt{default.prompt} na zestawie ~30 prostszych aplikacji webowych (zgodnie z listą w \texttt{experiments/h1.yaml}) oraz trzema modelami (gemini-2.5-flash, gpt-5-mini, deepseek-chat). Celem jest oszacowanie bazowego success rate i drop-off na etapach (build, apply, runtime) przy niskim koszcie. Porównanie z innymi wariantami promptów zostanie przeprowadzone w kolejnych przebiegach.

\bigskip
\noindent
TODO ???
Ze względu na koszt wywołań API oraz objętość danych, dla H1 przeprowadzane są 2 powtórzenia dla każdego repozytorium z trzema modelami faktycznie użytymi w eksperymencie (gemini-2.5-flash, gpt-5-mini, deepseek-chat) oraz oboma wariantami promptów.
