...
\textbf{RQ5 — podatność na manipulację kontekstem.} Skuteczność manipulacji na poziomie 56{,}7\% (zakres: 6{,}7\% dla \texttt{gemini-2.5-flash} — 100{,}0\% dla \texttt{deepseek-chat}) dowodzi, że agenty LLM nie posiadają wbudowanego mechanizmu weryfikacji spójności między kodem a dokumentacją. Model nie pyta „czy ten wymóg jest spójny z kodem?" — traktuje dostępny kontekst jako zbiór faktów równorzędnych. Z perspektywy bezpieczeństwa oznacza to, że autonomiczny agent działający na niezaufanym repozytorium jest podatny na ataki bez żadnego exploitowania podatności modelu — wystarczy dostęp do pliku dokumentacyjnego.
...
\textbf{Odporność na manipulację (RQ5).} Tu różnice są dramatyczne. \texttt{Gemini-2.5-flash} wykazał wyjątkową odporność: podatność na poziomie 6{,}7\% (2/30 przebiegów). Model \texttt{gpt-5-mini} był podatny w 63{,}3\% przypadków, a \texttt{deepseek-chat} uległ manipulacji we wszystkich 30 przebiegach (100\%). Tak skrajna różnica prawdopodobnie wynika z odmiennych strategii trenowania i wyrównania preferencji modelu w kontekście konfliktujących sygnałów — np.\ różnic w RLHF lub sposobie traktowania plików dokumentacyjnych jako kontekstu nieautorytatywnego — choć bez dostępu do procesu trenowania żadnej z tych hipotez nie można zweryfikować.
...
Eksperyment RQ4 objął dwa modele — \texttt{gemini-2.5-flash} i \texttt{deepseek-chat} — z wyłączeniem \texttt{gpt-5-mini}, który nie wspiera parametru \texttt{temperature=0} na poziomie API (szczegóły w sekcji~\ref{sec:rq4-design} rozdziału~4).
...
Wyniki RQ5 wskazują, że podatność agentów LLM na manipulację kontekstem jest realna i praktycznie istotna. Krytyczne odchylenia od specyfikacji wystąpiły w 56{,}7\% przypadków (od 6{,}7\% dla \texttt{gemini-2.5-flash} do 100{,}0\% dla \texttt{deepseek-chat}) przy stosunkowo prostych technikach manipulacji dokumentacyjnej — bez użycia zaawansowanych metod inżynierii promptów ani exploitowania podatności modelu.

\textbf{Mechanizm podatności.} Agenty LLM działające na repozytoriach traktują dostępne pliki — dokumentację, komentarze, pliki README, opisy API — jako wiarygodny kontekst. W środowisku produkcyjnym nie istnieje separacja między „zaufanymi" plikami kodu a „niezaufanymi" plikami dokumentacyjnymi. Oznacza to, że atakujący z prawem zapisu do repozytorium może osadzić w dokumentacji fałszywe wymagania, które agent bezkrytycznie przeniesie do generowanej konfiguracji wdrożeniowej. Jest to konkretna instancja \textit{indirect prompt injection} \cite{greshake_not_2023} w kontekście IaC — zjawiska opisanego również w szerszym przeglądzie bezpieczeństwa agentów LLM \cite{zhao_survey_2025}.
