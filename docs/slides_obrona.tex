...
\begin{frame}{RQ1 --- Typowe przyczyny awarii}
    \begin{columns}[T]
        \column{0.48\textwidth}
        \textbf{Etap build:}
        \begin{itemize}
            \item \texttt{npm ci} bez \texttt{package-lock.json} lub z zależnościami dev oznaczonymi nieprawidłowo (brak \texttt{vite} w runtime)
            \item Niezgodna wersja środowiska (Node 18 nie wspiera zależności aplikacji; Python \textless{} 3.10 i składnia \texttt{X | None})
            \item Błędy relatywnych importów Python (\texttt{ImportError: attempted relative import})
        \end{itemize}
        \vspace{0.4em}
        \textbf{Etap runtime:}
        \begin{itemize}
            \item Brak lub błędna zmienna środowiskowa z adresem DB (\texttt{could not translate host name})
            \item Literalne \texttt{"\$(POSTGRES\_USER)"} zamiast wartości (\texttt{password authentication failed})
        \end{itemize}

        \column{0.48\textwidth}
        \textbf{Etap K8s apply:}
        \begin{itemize}
            \item Brak nazw portów w \texttt{Service} z wieloma portami (\texttt{spec.ports[*].name: Required})
            \item Brak \texttt{Service} dla zależności (np. \texttt{notes-db})
        \end{itemize}
        \vspace{0.4em}
        \textbf{Etap build $\to$ runtime:}
        \begin{itemize}
            \item \texttt{runAsNonRoot} w K8s przy obrazie uruchamianym jako root
        \end{itemize}
        \vspace{0.6em}
        \begin{block}{}
            \small Dominują błędy \textbf{semantyczne} (środowisko, zależności), nie składniowe --- niewidoczne dla walidacji statycznej.
        \end{block}
    \end{columns}
\end{frame}
...
\begin{frame}{RQ5 --- Podatność na manipulację kontekstem}
    \begin{columns}[T]
        ...
        \vspace{0.3em}
        {\scriptsize
        W1: sprzeczna specyfikacja\\
        W2: autorytatywne zał. bezpieczeństwa\\
        W3: pozorna zależność historyczna}
...
\begin{frame}{Porównanie modeli}
    \begin{columns}[T]
        ...
        \begin{axis}[
            ...
            legend style={font=\tiny, at={(0.5,-0.28)}, anchor=north},
            legend columns=2,
            ylabel style={font=\footnotesize},
            ylabel={\%},
        ]
        ...
        \legend{RQ1: skuteczność runtime, RQ5: podatność na manipulację}
        \end{axis}
        \end{tikzpicture}

        \column{0.39\textwidth}
        \textbf{Skuteczność (RQ1):}
        \begin{itemize}
            \item GPT-5 Mini: \textbf{68,0\%}
            \item Gemini 2.5 Flash: 62,0\%
            \item DeepSeek V3.2: 58,0\%
            \item Różnica między modelami umiarkowana ($\leq$10 p.p.)
        \end{itemize}
        \vspace{0.5em}
        \textbf{Podatność (RQ5):}
        \begin{itemize}
            \item Gemini: \textbf{6,7\%} --- wyraźnie odporny
            \item GPT-5 Mini: 63,3\%
            \item DeepSeek: \textbf{100\%} --- zawsze manipulowalny
            \item Różnica krytyczna: 93 p.p.
        \end{itemize}
    \end{columns}
\end{frame}
