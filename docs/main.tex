...
5. **Security - Non-Root User** (RECOMMENDED, not required):
   - PREFER running containers as non-root user when possible, but root is acceptable if the application requires it
   ...
   - If running as non-root is feasible:
     ...
     * For nginx: either use nginx:alpine as root (simpler) or nginxinc/nginx-unprivileged (non-root, port 8080)
   - If the application requires root, omit the USER directive and let it run as root (default)
...
1. **Security Configuration Approach**:
   - PREFER non-root execution when the application supports it, but root is acceptable if needed
...
2. **When to use non-root security context**:
   ...
   - Using images designed for non-root (e.g., nginxinc/nginx-unprivileged)
